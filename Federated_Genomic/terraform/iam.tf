...
# Batch runner needs agent reporting capabilities (for Batch Agent)
...
# -----------------------------------------------------------------------------
# Federated Learning Demo - Cross-Project Permissions
# -----------------------------------------------------------------------------
# These permissions allow batch jobs to:
# 1. Pull container images from the central Artifact Registry
# 2. Write model weights to the shared GCS bucket

# Batch runner needs BigQuery access for training data
resource "google_project_iam_member" "batch_runner_bq" {
  project = var.project_id
  role    = "roles/bigquery.dataViewer"
  member  = "serviceAccount:${google_service_account.batch_runner.email}"
}

resource "google_project_iam_member" "batch_runner_bq_job" {
  project = var.project_id
  role    = "roles/bigquery.jobUser"
  member  = "serviceAccount:${google_service_account.batch_runner.email}"
}

# Batch runner needs storage write access for model weights
resource "google_project_iam_member" "batch_runner_storage_write" {
  project = var.project_id
